...
class GsSlotMap : public GsSlotMapBase
 { public:
   ...
   ~GsSlotMap () { while(_data.size()) delete (X*) _data.pop(); }
	...
	X* operator() ( int i ) const { return (X*)GsSlotMapBase::cget(i); }

	/*! Copy operator */
	void operator = ( const GsSlotMap& s ) { copy ( s ); }

	/*! Output */
	friend GsOutput& operator<< ( GsOutput& out, const GsSlotMap<X>& s )
	 { out<<"[ ";
	...
	/*! Input */
	friend GsInput& operator>> ( GsInput& inp, GsSlotMap<X>& s )
	 { s.init ();
	   inp.get(); // '['
