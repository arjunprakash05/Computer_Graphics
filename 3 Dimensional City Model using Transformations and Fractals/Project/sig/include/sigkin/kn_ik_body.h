...
class KnIkBody : public GsShareable
 { private :
	...
	union { KnIk* _ik[4];		 // larm, rarm, lleg, rleg (some may be null)
			KnHoapIk* _hik[4];	// larm, rarm, lleg, rleg (all will be valid)
		  };
	bool _hoapik;				 // true if in HOAP ik mode, false otherwise
	union { KnIk::Result _result; // last ik result
			KnHoapIk::Result _hresult;
		  };
	...
	char _osearch_active[4];	  // if not active, oserach.init is used as fixed angle
   ...
   ~KnIkBody ();
	...
	/*! Initializes specific IK solver for the HOAP skeleton.
		A fatal error will occur if the skeleton is not compatible */
	void init_hoap_ik ( KnSkeleton* sk );

	/*! Returns true if solving the IK for the HOAP, and false otherwise. */
	bool hoapik () const { return _hoapik; }

	/*! Returns ik of given index: 0:larm, 1:rarm, 2:lleg, 3:rleg. Can be null. */
	KnIk* ik ( int i ) { return _hoapik? 0:_ik[i]; }

	/*! Returns the hoap ik of given index: 0:larm, 1:rarm, 2:lleg, 3:rleg. Can be null. */
	KnHoapIk* hik ( int i ) { return _hoapik? _hik[i]:0; }
	...
	KnHoapIk::Result hresult () const { return _hresult; }
