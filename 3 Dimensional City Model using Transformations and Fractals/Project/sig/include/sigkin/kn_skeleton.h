...
class KnSkeleton : public GsShareable
 { protected :
   ...
   public :
	...
	/*! Set if limits in a local parameterization should enforce rotation values set by a
		a different parameterization or by the joints quaternion. */
	void enforce_param_limits ( bool epl ) { _enforce_param_limits=epl; }

	/*! Returns cross-local parameterization limits enforecement state. */
	bool enforce_param_limits () const { return _enforce_param_limits; }
...
inline void global2local ( const GsMat& gm, GsMat& lm, const GsMat& frame )
 { lm.mult ( gm, frame.inverse() ); }

/* inline utility to remind how to convert local to global coordinates */
inline void local2global ( const GsMat& lm, GsMat& gm, const GsMat& frame )
 { gm.mult ( lm, frame ); }
