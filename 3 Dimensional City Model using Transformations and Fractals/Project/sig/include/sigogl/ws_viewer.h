...
class WsViewer : public WsWindow 
 {
   ...
   ~WsViewer ();
	...
	/*! Set a text to appear in the status bar of the graphics window. 
		Use s==0 to erase current message. */
	void message ( const char* s );

	/*! Access the string of the status bar message text. */
	GsString& message ();
	...
	/*! Increments the scene rotation in the used Camview by dq, and makes sure
		the rotation is normalized after a certain (large) number of increments */
	void increment_model_rotation ( const GsQuat &dq );
   ...
   protected : //----> methods overriding WsWindow virtual methods
	...
	/*! handle() will respond to scene-related mouse events and will then call the
	...
	/*! uievent() handles commands generated by the menu attached to a right-button
