...
void KnIk::base_frame ( GsMat& bframe )
{
	...
	jmat.setl4 ( _base->offset() );

	if ( _base->parent() )
	{	bframe.mult ( jmat, _base->parent()->gmat() );
	}
}
...
{  
	...
	*/
	...
	// End point:
	GsPnt e ( goal[12], goal[13], goal[14] );
	...
	{	GsMat bframe;
		base_frame ( bframe );
		GsPnt shoulderp ( bframe[12], bframe[13], bframe[14] ); // _base joint in global coords
		...
		lines.push ( shoulderp, e*bframe ); // _base-end line
		...
		lines.push ( GsColor::cyan );
		if ( r>0 ) lines.push_circle_approximation ( c*bframe, u*r*bframe3x3, n*bframe3x3, 60 ); // orbit circle
		lines.push ( GsColor::cyan );
		lines.push ( c*bframe, m*bframe ); // circenter-mid line
		lines.push ( GsColor::magenta );
		lines.push ( shoulderp, m*bframe ); // _base-mid line
