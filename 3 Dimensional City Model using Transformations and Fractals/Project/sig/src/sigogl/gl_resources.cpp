...
const GlTexture* GlResources::get_texture ( int txid )
{
	...
	{	GlTextureDecl* td = t->_decl;
		...
		if ( !ws_ogl_loaded() ) Error("get_texture called before OpenGL loaded",fname);
...
const GlFont* GlResources::get_font ( int fontid )
{
	...
	{	lfnt = GlFont::default_font(); 
		if ( !lfnt->texture() && ws_ogl_loaded()>0 ) // send texture to OpenGL
	...
	{	GlFontDecl* fd = f->_decl;
		...
		if ( ws_ogl_loaded()>0 ) // load and send texture to OpenGL
