...
void UiManager::update_style ()
{
	...
	_win->activate_ogl_context ( true );
...
void UiManager::focus ( UiElement* w )
{
	GS_TRACE1("focus...");
	...
	{	if (_focus)
		{	if ( _focus->panel()&&_focus->submenu() ) close_submenus( ((UiPanel*)_focus) );
			_focus = 0;
		}
...
int UiManager::handle ( const GsEvent& e )
{
	...
	{	if ( rbutton_panel_active()&&_submenus==0 ) { hide_rbutton_panel(); _rbmx=e.mousex; _rbmy=e.mousey; }
		_newevent=false; _win->uievent(_lastevent);
	}
