...
void UiPanel::center_children ()
{
	...
	{	int i;
		float w, maxw = 0;
		for ( i=0; i<elements(); i++ ) // get max width
		{	w = get(i)->rect().w;
			if ( w>maxw ) maxw=w;
		}
...
{
	if ( e->type()==t ) // store range of same type of elements
	{	if ( g.empty() )
		...
		{	if ( ea[g.top()]->type()!=t ) g.pop(); // different type: not a new range
			else if ( g.top()+1==i ) g.push()=i; // sequential type: new range starting
			else g.pop(); // not sequential: not a new range
		}
		else if ( g.size()%2==0 )
...
void UiPanel::build ()
{
	...
	if ( _xclose>0 )
	{	if ( _label.text() && _label.align()==UiLabel::Right ) _label.center();
	...
	{	e = get(i);
		...
		{	check_grouped_elements ( _elements, e, i, UiElement::Input, egroups );
			check_grouped_elements ( _elements, e, i, UiElement::Slider, egroups );
			ih += e->nextspc();
			if ( i+1<s )
			{	ih += get(i+1)->priorspc();
	...
	{	if (_label.text()) UiElement::resize(-1,_rect.h);
		for ( i=0; i<elements(); i++ ) get(i)->resize(0,_rect.h-ydspc);
	}
	else // vertical
	{	if (_label.text()) UiElement::resize(_rect.w,-1);
		for ( i=0; i<elements(); i++ ) get(i)->resize(_rect.w-xdspc,-1);
	}
	...
	{	GS_TRACE3 ( "Aligning grouped elements: "<<egroups );
		...
		for ( int k=0; k<egroups.size(); k+=2 )
		{	int lmax=0;
