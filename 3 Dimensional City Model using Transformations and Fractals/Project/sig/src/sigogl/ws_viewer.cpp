...
{  public :
	...
	int spincount;			// counts increments applied to spin animation (for normalization)
	...
	GsMat matm, matp;		// Transformation matrices used when rendering with shaders
...
UiOutput* WsViewerData::output ()
{
	// Output element ===============================================================
	if ( !_output )
	{	UiPanel* p = rbpanel->uimparent()->add_panel ( 0, UiPanel::Vertical, 80, 40 );
...
UiOutput* WsViewerData::message ()
{
	// Message bar ==================================================================
	if ( !_message )
	{	UiPanel* p = rbpanel->uimparent()->add_panel ( 0, UiPanel::HorizLeft, UiPanel::BottomBar );
		p->color().ln.a=0; // no frame
		p->color().bg.a=0; // no background
...
# define USERSROOT 1
# define SCENEBOX  ((SnLines*)_data->vroot->get(2))
		 ...
		 :WsWindow ( x, y, w, h, label )
{
	...
	_data->vroot->add ( _data->uroot ); // user scene
	_data->vroot->add ( new SnLines );  // bbox
	_data->vroot->compress();
...
GsBox WsViewer::update_bbox( GsBox* boxpt )
{
	...
	if ( boxpt )
	{	b = *boxpt; 
		_data->bboxaction.set(b);
	}
	else
	{	_data->bboxaction.apply ( _data->uroot );
		b = _data->bboxaction.get();
	}
	SCENEBOX->init();
	SCENEBOX->push_box ( b, true );
	return b;
}
...
void WsViewer::view_node ( SnNode* n, float fovy )
{
	...
	cv.translation = -center;
	cv.znear = 0.01f;
	cv.zfar  = 3.1f;// + size/2.0f;
	cv.fovy = fovy;
...
void WsViewer::view_all ( GsBox* boxpt )
{
	...
	{	GsBox box = update_bbox ( boxpt );
		if ( !box.empty() )
		{	// if ( SCENEAXIS->visible() ) box.extend(GsPnt::null);
			GsVec center = box.center();
			if ( box.maxsize()<0 ) center=GsVec::null;
			float alen = update_axis(&box);
			float size = alen * 1.2f; //was: sqrtf(2.0f)*alen;
			if ( size<0.0001f ) size=0.0001f;
//CamDev: restric use only of camera parameters:
if (0){ // SgDev: complete changing camera's transformations to camera-only transformations, then rename GsCamera to GsCamera
			cv.scale = 1.0f / size;	// we want to eliminate scaling
			cv.translation = -center; // and translation
			cv.znear = 0.01f;
			cv.zfar  = 3.1f;
//_data->light.position.z = 0.01f;
}else{	// Solution below is a start, still need to change all the camera manipulation methods
			cv.fovy = GS_TORAD(65.0f);
			float s = box.maxsize()/2.0f;
			float d = s/tanf(cv.fovy/2.0f);
			cv.center = box.center();
			cv.eye = cv.center+GsVec(0,0,d+s); // s here should be box center dist to box front plane
			cv.znear = 0.01f;
			cv.zfar  = d+s+s;

			cv.scale = 1.0f;
			cv.translation.set(0,0,0);
}
			if ( aspect<1.0f ) cv.scale*=aspect;
		}
	}
...
void WsViewer::increment_model_rotation ( const GsQuat &dq )
{
	//CamDev: camera.rotation is being removed - update this method:
	GsQuat& rot = _data->camera.rotation;
	rot = dq*rot;

	_data->spincount++;
	if ( _data->spincount%1000==0 )
	{ _data->spincount = 0;
	  _data->spindata.rotdelta.normalize();
	  rot.normalize();
	}
}
...
void WsViewer::draw ( GlRenderer* wr ) 
{
	...
	GsMat& m = _data->matm;
	GsMat& p = _data->matp;
	cv.getmat ( m, p ); // CamDev: will only have 1 matrix, the projection matrix
	_data->vr->init ( &p, &m );
...
int WsViewer::handle ( const GsEvent &e )
{
	...
	{	GsCamera& c = _data->camera; // compute the ray crossing the scene
		GsPlane plane ( c.center, c.rotation.inverse().apply(GsVec::k) );
...
int WsViewer::handle_examiner_manipulation ( const GsEvent &e )
{
	...
	{ 
		c.fovy += float(e.wheelclicks)*_data->zoomfactor;
	...
	{ 
		...
		if ( ZOOMING(e) )
		{	float factor = -_data->bboxaction.get().maxsize();
...
int WsViewer::handle_planar_manipulation ( const GsEvent& e )
{
	...
	if ( e.type==GsEvent::Wheel ) // ZOOM
	{	float factor = 0.05f;
		c.fovy += float(e.wheelclicks)*factor;
	...
	{ 
		...
		{	float factor = 0.02f;
			c.fovy += (e.mousedx()+e.mousedy())*factor;
			c.fovy = GS_BOUND ( c.fovy, 0.001f, gspi );
		}
		else if ( TRANSLATING(e) ) // this will translate in planar mode
		{
			c.eye -= e.mousedp(); c.center -= e.mousedp();
		}
		else if ( ROTATING(e) ) // planar mode
		{	GsQuat dq;
			GsPnt p1 ( e.lmousep.x, e.lmousep.y, 0.0f );
			GsPnt p2 ( e.mousep.x, e.mousep.y, 0.0f );
			dq.set ( p1, p2 );
			increment_model_rotation ( dq );
		}
		else { return 0; }
...
int WsViewer::key_cmd ( char c )
{
	...
	{	case 'p' : gsout<<gsnl<<_data->camera<<gsnl; return 1;
		...
		case 'r' : activate_ogl_context(true); GlResources::print_report(); return 1;
		case 'x' : ws_exit ();
	}
	return 0;
}
