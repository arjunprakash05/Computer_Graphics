...
class GsPolygon : public GsArray<GsPnt2>
 { private :
   ...
   public :
	...
	/*! Divides the polygon in triangles, returning triangle indices to vertices 
		of vector v. The GsPolygon must be in CCW orientation. If not, its vertex
		ordering should be reversed before calling this function. Parameter prec
		is used to join vertices that are too close and to remove almost collinear 
		vertices in order to improve the robustness of the triangulation. */
