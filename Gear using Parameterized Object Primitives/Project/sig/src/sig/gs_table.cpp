...
int GsTableBase::lookup_index ( const char *key ) const
 {
   ...
   while ( true )
	{ if ( gs_compare(_table[id].key,key)==0 ) return id; // already there
...
bool GsTableBase::insert ( const char *key, void* data )
 {
   ...
   while ( true )
	{ if ( gs_compare(_table[id].key,key)==0 ) // already there
	   { _last_id = id;
		 return false;
	   }
...
void* GsTableBase::remove ( const char *key )
 {
   ...
   int priorid=id;
   while ( true )
	{ if ( gs_compare(_table[id].key,key)==0 ) // found: remove it
	   { 
