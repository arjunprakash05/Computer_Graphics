...
int SnPolyEditor::handle_event ( const GsEvent &e, float t )
{
	...
	GsVec2 p ( e.mousep.x, e.mousep.y );
	GsVec2 lp ( e.lmousep.x, e.lmousep.y );
	...
	{	case ModeAdd:
		...
		{	if ( e.type==GsEvent::Push )
			...
			else if ( e.type==GsEvent::Drag && _selvtx>-1 )
	...
	{	case ModeAdd:
		{	if ( e.key==GsEvent::KeyDel || e.key==GsEvent::KeyBack )
			...
			{	GS_TRACE1 ( "Handling Esc..." );
				...
				if ( !p.ccw() ) p.reverse(); // put in CCW orientation
				mode ( ModeAdd );
		...
		{	if ( e.key==GsEvent::KeyEsc ) { mode(ModeAdd); return 1; }
			if ( e.key==GsEvent::KeyDel && _selvtx>-1 )
			{	GsPolygons& p = *_polygons->polygons();
...
int SnPolyEditor::check_event ( const GsEvent& e, float& t )
{
	...
	t = -1;
	return 2; // right now will consume all meaningful events, could extend to check against a given boundary
}
