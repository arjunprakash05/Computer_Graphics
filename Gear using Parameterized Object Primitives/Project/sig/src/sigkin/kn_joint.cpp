...
static void _unite ( KnJoint* j, GsModel& m, GsModel& tmp, const GsMat& parmat, bool ifvisgeo )
 {
   ...
   if ( geo )
	{ tmp = *geo;
	  tmp.transform ( parmat );//j->gmat() );
	  m.add_model ( tmp );
	}

   int i;
   for ( i=0; i<j->children(); i++ )
	{ GsMat jmat = j->child(i)->lmat() * parmat;
...
bool KnJoint::ikinit ( KnIk::Type t )
 {
   ...
   if ( rot_type()==KnJoint::TypeEuler )
	{ _ik = new KnHoapIkSolver();
	}
   else
	{ _ik = new KnStdIkSolver();
	}
   _ik->ref();
...
KnIk::Result KnJoint::iksolve ( const GsVec& pos )
 {
...
KnIk::Result KnJoint::iksolve ( const GsVec& p, const GsQuat& q )
 {
   if ( !_ik ) return KnIk::Undef;
...
KnIk::Result KnJoint::iksolve ( const GsMat& m )
 {
   if ( !_ik ) return KnIk::Undef;
   _ik->solve_rot_goal ( true );
   _ik->goal = m;
