...
void gl_clear_color ( const GsColor& c )
 {
   // glClearColor (v4.5) Specify the rgba values used when the color buffers are cleared
...
void gl_print_info ( GsOutput* out )
 {
   ...
   o << "  OpenGL: "  <<  gl_version() << ", \""<< (const char*)glGetString(GL_VERSION) << end;
   o << "	GLSL: "  << glsl_version() << ", \""<< (const char*)glGetString(GL_SHADING_LANGUAGE_VERSION) << end;
 }
...
int gl_version () 
 { 
   static int v=-1;
   if ( v<0 ) v = extractversion ( glGetString(GL_VERSION) );
   return v;
 }
...
int glsl_version () 
 { 
   static int v=-1;
   if ( v<0 ) v = extractversion ( glGetString(GL_SHADING_LANGUAGE_VERSION) );
   return v;
 }
