	...
	{	char delc=0;
		...
		{	switch (e.key)
			{ case GsEvent::KeyLeft:  if (_cursor>0) _cursor--; if (_cursor<_curini)_curini--; break;
			  ...
			  case GsEvent::KeyEnter: _cursor=_curini=0; _inuse=0; uim->uievent(_event,this); uim->focus(0); break;
			  default:	if (e.character==0) break;
						_text.insert(_cursor, 1);
						_text[_cursor++] = e.character == '\t' ? ' ':e.character;
...
void UiInput::draw ( UiPanel* panel )
{
	...
	{	// draw edition bar:
		...
		GsColor lc = cs.selection_fg;
		ctext = cs.selection_fg;
		pos->push_line ( GsPnt2(cp.x,_ir.y+1), GsPnt2(cp.x,_ir.y+_ir.h-1), 1.0f, lc );
	}
