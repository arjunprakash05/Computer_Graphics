...
void WsWindow::activate_ogl_context ( bool state ) const { wsi_activate_ogl_context(_syswin,state); }
...
WsWindow::WsWindow ( int x, int y, int w, int h, const char *label, Mode m )
{
	...
	wsi_new_win ( x, y, w, h, label? label:"", this, _syswin, m );
	_w = w;
	_h = h;
...
bool WsWindow::snapshot ( const char* filename )
{
	...
	if ( !filename ) 
	{	filename = ui_input ( "Enter png/tga/bmp file name to save:", "./frame.png" );
		if ( filename ) { redraw(); ws_check(); } // to close dialog
		else return false;
	}

	GsImage image;
	wsi_win_setoglcontext ( _syswin, true );
	gl_snapshot ( image );
	wsi_win_setoglcontext ( _syswin, false );
