...
{ public :
  ...
  public : // construction functions:
	...
	/*! Clear materials and then set M and G so that all triangles use the
		same material m, with compression and MtlMode update. */
	void set_one_material ( const GsMaterial& m, const char* name=0 );
	...
	/*! Empty G array and adjust MtlMode. */
	void clear_groups ();

	/*! Organize materials per groups in G from per-face material indices to array M */
	...
	/*! Clear the N and Fn arrays, with compression, then adjust mode. */
	void flat ();
  ...
  public : // query functions:
	...
	/*! Sequentially stores in fn, for all faces, the 3 normals for each vertex of each face in the model. 
		...
		then, in case N.size()==V.size() the normals in N are used, otherwise, flat normals are returned.
