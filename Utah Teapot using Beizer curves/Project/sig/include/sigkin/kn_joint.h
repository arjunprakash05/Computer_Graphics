...
{  public :
   ...
   public :
	...
	/*! Initialize the ik solver, allocating it if needed, ref/unref rules followed.
		If this joint is of Euler type, a hoap ik solver will be initialized. */
	bool ikinit ( KnIk::Type t );

	/*! Access the ik solver; will be null if none is associated */
	KnIkSolver* ik () { return _ik; }

	/*! Solve an apply the ik to the given target position in base coordinates */
	KnIk::Result iksolve ( const GsVec& p );

	/*! Solve an apply the ik to the given target position and orientation in base coordinates */
	KnIk::Result iksolve ( const GsVec& p, const GsQuat& q );

	/*! Solve an apply the ik to the given target position and orientation in base coordinates */
	KnIk::Result iksolve ( const GsMat& m );
