...
{  public :
   ...
   public :
	...
	void init () { init_matrix(); _context->projection(&GsMat::id); _context->modelview(&_matstack[0]); }
	...
	{ init_matrix(); _context->projection(p); _context->modelview(&_matstack[0]); }

	/*! Initializes the matrix stack and the context with given transformations. */
	void init ( const GsMat* p, const GsMat* c )
	{ init_matrix(*c); _context->projection(p); _context->modelview(&_matstack[0]); }
