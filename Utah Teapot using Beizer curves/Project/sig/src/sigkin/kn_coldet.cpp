...
void KnColdet::init ()
 {
   SkelData* cs;
   int i, id, maxid=_skels.maxid();
   for ( id=0; id<=maxid; id++ )
	{ cs = (SkelData*)_skels.get(id);
	  if ( !cs ) continue;
	  cs->s->_coldetid = -1;
	  for ( i=0; i<cs->j.size(); i++ ) cs->j[i]->_coldetid = -1;
	  delete cs;
	}
   _skels.init_arrays();
   _coldet->init();
...
int KnColdet::connect ( KnSkeleton* s, GsArray<KnJoint*>* pairs, bool deactivate_adjacent )
 {
   SkelData* cs;
   ...
   size = colids.size();
   for ( i=0; i<size; i+=2 ) // deactivate reported pairs
	_coldet->deactivate_pair ( colids[i], colids[i+1] );
