...
void KnIkBody::_unrefik ()
 {
   int i;
   if ( _hoapik )
	{ for ( i=0; i<4; i++ ) { _hik[i]->unref(); _hik[i]=0; } }
   else
	{ for ( i=0; i<4; i++ ) if ( _ik[i] ) {_ik[i]->unref(); _ik[i]=0;} }
   _hoapik = false;
 }
...
void KnIkBody::_initskel ()
 {
   ...
   int i;
   if ( _hoapik )
	{ _skeleton = _hik[0]->base()->skeleton(); }
   else
	{ for ( i=0; i<4; i++ ) { if ( _ik[i] ) break; } 
	  if ( i<4 ) _skeleton = _ik[i]->base()->skeleton();
	}
...
void KnIkBody::init_hoap_ik ( KnSkeleton* sk )
 {
   _unrefik ();
   
   for ( int i=0; i<4; i++ )
	{ _hik[i] = new KnHoapIk ( (KnHoapIk::Type)i, sk );
	  _hik[i]->ref();
	}
   _hoapik = true;
   _initskel ();
...
void KnIkBody::remove_joint_limits ()
 {
   if ( _hoapik )
	{ _hik[0]->remove_joint_limits();
	  _hik[1]->remove_joint_limits();
	  _hik[2]->remove_joint_limits();
	  _hik[3]->remove_joint_limits();
	}
   else
...
void KnIkBody::capture_init_posture ()
 {
   ...
   if ( _hoapik )
	{ _lfoot = _hik[2]->end()->gmat();
	  _rfoot = _hik[3]->end()->gmat();
...
KnIk::Result KnIkBody::solve ( int i, const GsMat& globgoal )
 {
   if ( _hoapik )
	{
	  _posture.get(); // save body posture
	  _skeleton->update_global_matrices();
	  _locgoal = globgoal;
	  _hik[i]->set_local(_locgoal);

	  if ( _solvetime>=0 ) _solvetime=gs_time();
	  _hresult = _hik[i]->solve ( _locgoal, _osearch[i].init, _coldet );
	  if ( _solvetime>=0 ) _solvetime=gs_time()-_solvetime;
	  if ( _result==KnIk::Ok )
	   _hik[i]->apply_last_result();
	  else
	   _posture.apply();
	  return _result;
	}
