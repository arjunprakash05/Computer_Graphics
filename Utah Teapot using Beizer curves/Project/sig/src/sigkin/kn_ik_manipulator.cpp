...
bool KnIkManipulator::init ( KnHoapIk* hik, GsModel* goalmodel )
 {
   if ( !hik ) return false;
   if ( !hik->base() ) return false;

   // unref old classes:
   if ( _ik ) { _ik->unref(); _ik=0; }
   if ( _hik ) _hik->unref();
   if ( _ikbody ) { _ikbody->unref(); _ikbody=0; }
   ...
   // create nodes to display:
   GsModel* model = goalmodel? goalmodel:_hik->end()->visgeo();
   _manip = new SnManipulator;
   _lines = new SnLines;
   _matrix = new SnTransform;
   SnGroup::remove_all();
   ...
   SnGroup::add ( _lines );
   SnGroup::add ( _matrix );
   SnGroup::add ( new SnModel(model) ); // ok if pt is null

   // Create axis with the same sizes as of the model to manipulate:
   GsBox box;
   if ( model ) model->get_bounding_box ( box );
	else { box.a=GsVec::minusone; box.b=GsVec::one; box*=2.0f; }
   SnLines* l = new SnLines;
   l->push_axis ( GsPnt::null, 0/*len not used*/, 3, "xyz", true/*unit marks*/, &box );

   // Set the manip to control the axis in the current wrist place:
   _hik->end()->skeleton()->update_global_matrices();
   match ();
   _manip->child ( l );
   _manip->callback ( _ikcb, this );
...
bool KnIkManipulator::init ( KnIkBody* ikbody, int i, GsModel* goalmodel )
 {
   ...
   if ( ikbody->hoapik() )
	{
	  if ( !init(ikbody->hik(i),goalmodel) ) return false;
	}
   else
...
void KnIkManipulator::match ()
 {
   GsMat goal;
   if ( _hik )
	{ _hik->base()->skeleton()->update_global_matrices();
	  goal = _hik->end()->gmat();
	}
   else
...
KnIk::Result KnIkManipulator::iksolve ()
 {
   ...
   if ( _method==UserCallback )
	...
	{ GsMat goal = _manip->mat();
	  _ik->set_local ( goal );
	  res = _ik->solve ( goal, _iksearch.oangle, _coldet );
	  if ( res==KnIk::Ok )
	...
	{ GsMat goal = _manip->mat();
	  _ik->set_local ( goal );
	  res = _ik->solve ( goal, _iksearch, _coldet );
	  if ( res==KnIk::Ok )
	   { _ik->apply_last_result();
		 if ( _knscene ) _knscene->update();
		 if ( _knscene->skeleton()->skin() ) _knscene->skeleton()->skin()->update();
