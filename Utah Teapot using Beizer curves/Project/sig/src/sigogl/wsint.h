...
// This is an internal header file with the wrapper functions to connect to the window system
...
void wsi_new_win ( int x, int y, int w, int h, const char* label, WsWindow* uwindow, void*& win, int mode=0 );
...
// activate (or deactivate) the opengl context of the window
...
void wsi_activate_ogl_context ( void* win, bool state );
...
// loads OpenGL core functions - includes self-protection to load functions only the first time called
void wsi_load_ogl ();

// Returns the number of OpenGl functions which have been loaded, 0 if none loaded.
