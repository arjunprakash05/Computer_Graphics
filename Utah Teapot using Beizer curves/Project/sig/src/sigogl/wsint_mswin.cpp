...
static MSG			AppMsg;
static char*		AppName="GKiMAppClass";
...
void wsi_new_win ( int x, int y, int w, int h, const char* label, WsWindow* uwindow, void*& win, int mode )
{
...
static LRESULT CALLBACK WndProc ( HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam )
{
	...
	{	GS_TRACE5 ( "WM_PAINT ["<<sw->label<<"]..." );
		...
		if (sw->needsinit)
		{	wsi_load_ogl (); // only overall 1st call will actually load ogl
			int w, h; 
			wsi_win_size ( sw, w, h ); 
